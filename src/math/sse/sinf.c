...
#define SSE_NAME bN2v_sin

#include "../commonf2d/sse.c"

/*This function deal with size in 1~3*/
__attribute__((always_inline)) inline void sse_3(const float *input_array,
...
__attribute__((always_inline)) inline void sse_7(const float *input_array,
                                                 float *result_array, int size,
                                                 int *array_index)
{
  if (size == 7)
...
__attribute__((always_inline)) inline void sse_15(const float *input_array,
                                                  float *result_array, int size,
                                                  int *array_index)
{
  if (size == 15)
